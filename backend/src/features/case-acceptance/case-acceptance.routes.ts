...
import {
  ...
  checkCaseAcceptanceDuplicateSchema,
...
// POST /api/case-acceptance/check-duplicate — pre-flight for the entry form.
// POST (not GET) because it takes the whole natural key as a body. Read-only.
router.post('/check-duplicate', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const body   = checkCaseAcceptanceDuplicateSchema.parse(req.body);
    const report = await caseAcceptanceService.checkDuplicate(req.scope!, body);
    res.json(report);
  } catch (err) { next(err); }
});
